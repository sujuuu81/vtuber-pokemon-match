...
type Result = { ranking: { name: string; score: number; reason: string }[]; analysis?: { hair:string; eyes:string; colors:string[]; outfit:string; silhouette:string; theme:string } };
...
export default function Home() {
  ...
  return <main className="page"><div className="shell"><header className="hero"><div className="eyebrow">AI × VTUBER × POKÉMON</div><h1>VTuberポケモン診断</h1><p>REALITYなどのアバター全身画像から、<br/>見た目が一番似ているポケモンをAIが診断します。</p></header><section className="card">
    ...
    {loading&&<div className="loading">AIがアバターの外見を分析しています…<br/><span className="muted">髪・目・衣装・色・シルエットなどを比較中</span></div>}
    {result&&<div className="result"><div className="eyebrow">YOUR POKÉMON</div><h2>{result.ranking[0].name}</h2><div className="score">見た目の一致度 {result.ranking[0].score}%</div><div className="reason"><strong>AIが選んだ理由</strong><br/>{result.ranking[0].reason}</div><h3>似ているポケモン TOP5</h3><div className="rank">{result.ranking.map((x,i)=><div className="rankrow" key={x.name}><span>#{i+1}　{x.name}</span><strong>{x.score}%</strong></div>)}</div><div className="actions" style={{marginTop:20}}><button className="share" onClick={share}>𝕏 結果を投稿</button><button className="choose" onClick={()=>{setResult(null);setFile(null);setPreview('')}}>もう一度診断</button></div></div>}
